...
// TemplateSettings constructor
TemplateDetector::TemplateSettings::TemplateSettings()
    : matchThreshold(0.7f)
    , minScale(0.5f)
    , maxScale(2.0f)
    , scaleStep(0.1f)
    , templateMethod(cv::TM_CCOEFF_NORMED)
    , useMultipleTemplates(true)
    , maxRotationDegrees(15)
    , rotationStep(5) {}

// TemplateData constructor
TemplateDetector::TemplateData::TemplateData() : aspectRatio(1.0f) {}

// MatchResult constructor
TemplateDetector::MatchResult::MatchResult() {}

// TemplateDetector constructor
TemplateDetector::TemplateDetector(const TemplateSettings& settings) 
    : settings_(settings) {
    initializeTemplates();
    LOG_INFO("Template matching detector initialized with ", templates_.size(), " templates");
}

Domain::DetectionResult TemplateDetector::detect(const Types::Image& image) {
    if (image.empty()) {
...
Types::DetectionMethod TemplateDetector::getMethod() const {
    return Types::DetectionMethod::TEMPLATE_MATCHING;
}

Types::ConfidenceScore TemplateDetector::getExpectedConfidence(const Types::Image& image) const {
...
bool TemplateDetector::isCapable(const Types::Image& image) const {
    return !image.empty() && !templates_.empty() && 
           image.cols >= 400 && image.rows >= 300;
}

std::string TemplateDetector::getName() const {
    return "Template Matching Detector";
}

void TemplateDetector::setTemplateSettings(const TemplateSettings& settings) {
    settings_ = settings;
}

TemplateDetector::TemplateSettings TemplateDetector::getTemplateSettings() const {
    return settings_;
}

void TemplateDetector::setConfidenceThreshold(float threshold) {
    confidenceThreshold_ = std::clamp(threshold, 0.0f, 1.0f);
}

float TemplateDetector::getConfidenceThreshold() const {
    return confidenceThreshold_;
}

bool TemplateDetector::loadCustomTemplate(const std::string& templatePath) {
    try {
...
void TemplateDetector::initializeTemplates() {
...
void TemplateDetector::createSyntheticTemplate() {
...
void TemplateDetector::createVariationTemplates() {
...
TemplateDetector::MatchResult TemplateDetector::findBestMatch(const Types::Image& image) {
    ...
    for (size_t templateIdx = 0; templateIdx < templates_.size(); ++templateIdx) {
        ...
        for (float scale = settings_.minScale; scale <= settings_.maxScale; scale += settings_.scaleStep) {
            ...
            if (newSize.width <= 0 || newSize.height <= 0 ||
                newSize.width > image.cols || newSize.height > image.rows) {
                continue; // Template too small/large or invalid size
            }
...
std::vector<cv::Point2f> TemplateDetector::calculateCorners(const MatchResult& match) {
...
std::vector<cv::Point2f> TemplateDetector::generatePatchCenters(const std::vector<cv::Point2f>& corners) {
...
std::vector<Domain::ColorPatch> TemplateDetector::extractColorPatches(const Types::Image& image,
...
float TemplateDetector::calculateFinalConfidence(const MatchResult& match, 
