#include "DetectionValidator.hpp"

namespace ColorCorrection::Internal::Detection {

// ValidationSettings constructor
DetectionValidator::ValidationSettings::ValidationSettings()
    : minColorCheckerArea(10000.0f)
    , maxColorCheckerArea(500000.0f)
    , minAspectRatio(1.2f)
    , maxAspectRatio(1.8f)
    , maxSkewAngle(15.0f)
    , minRequiredPatches(20)
    , maxPatchDistanceVariation(0.3f)
    , minPatchConfidence(0.3f)
    , maxDeltaE(50.0f)
    , maxBrightnessVariation(0.8f)
    , validateColorConsistency(true)
    , confidenceThreshold(0.6f)
    , minInlierPatches(18)
    , inlierThreshold(10.0f)
    , enableTemporalValidation(false)
    , maxFrameToFrameMovement(50.0f)
    , temporalConsistencyFrames(3) {}

// DetectionValidator constructor
DetectionValidator::DetectionValidator(const ValidationSettings& settings)
    : settings_(settings) {
    LOG_INFO("Detection validator initialized");
}

// Main validation method
DetectionValidator::ValidationResult DetectionValidator::validate(const Domain::DetectionResult& detection) {
...
Types::ConfidenceScore DetectionValidator::calculateConfidenceScore(const Domain::DetectionResult& detection) {
    ValidationResult validation = validate(detection);
    return Types::ConfidenceScore::fromValue(validation.overallScore);
}

bool DetectionValidator::isDetectionAcceptable(const Domain::DetectionResult& detection) {
    ValidationResult validation = validate(detection);
    return validation.isValid;
}

DetectionValidator::ValidationSettings DetectionValidator::getSettings() const {
    return settings_;
}

void DetectionValidator::setSettings(const ValidationSettings& settings) {
    settings_ = settings;
}

// Enhanced validation for specific use cases
DetectionValidator::ValidationResult DetectionValidator::validateForCalibration(const Domain::DetectionResult& detection) {
...
DetectionValidator::ValidationResult DetectionValidator::validateForRealTime(const Domain::DetectionResult& detection) {
...
float DetectionValidator::validateGeometry(const Domain::DetectionResult& detection, ValidationResult& result) {
    float score = 0.0f;
...
float DetectionValidator::validatePatchQuality(const Domain::DetectionResult& detection, ValidationResult& result) {
...
float DetectionValidator::validateColorConsistency(const Domain::DetectionResult& detection, ValidationResult& result) {
    const std::vector<Domain::ColorPatch>& patches = detection.getPatches();
...
float DetectionValidator::validateSpatialConsistency(const Domain::DetectionResult& detection, ValidationResult& result) {
    const std::vector<Domain::ColorPatch>& patches = detection.getPatches();
...
float DetectionValidator::calculateOverallScore(const ValidationResult& result) {
...
void DetectionValidator::populateStatistics(const Domain::DetectionResult& detection, ValidationResult& result) {
...
float DetectionValidator::calculateRectangularity(const std::vector<Types::Point2D>& corners) {
...
float DetectionValidator::calculateSkewAngle(const std::vector<Types::Point2D>& corners) {
...
float DetectionValidator::calculateSpacingConsistency(const std::vector<float>& spacings) {
