#include "CorrectionMatrixCalculator.hpp"
...
// CalculationSettings constructor
CorrectionMatrixCalculator::CalculationSettings::CalculationSettings()
    : method(CalculationMethod::SVD_LEAST_SQUARES)
    , enableRegularization(true)
    , regularizationFactor(1e-6f)
    , ridgeAlpha(0.01f)
    , enableOutlierRemoval(true)
    , outlierThreshold(2.0f)
    , minInlierPatches(18)
    , ransacIterations(1000)
    , ransacThreshold(1.5f)
    , ransacConfidence(0.99f)
    , enableCrossValidation(true)
    , crossValidationFolds(5)
    , maxConditionNumber(1000.0f)
    , minDeterminant(1e-6f)
    , useLinearRGB(true)
    , normalizeInputs(false)
    , constrainPositivity(false) {}

// CalculationResult constructor
CorrectionMatrixCalculator::CalculationResult::CalculationResult() {}

bool CorrectionMatrixCalculator::CalculationResult::isValid() const {
    return success && matrix.isValid() && 
           conditionNumber < 1000.0f && std::abs(determinant) > 1e-6f;
}

// CorrectionMatrixCalculator constructor
CorrectionMatrixCalculator::CorrectionMatrixCalculator(const CalculationSettings& settings)
...
CorrectionMatrixCalculator::CalculationResult CorrectionMatrixCalculator::calculateMatrix(const std::vector<Domain::ColorPatch>& patches) {
...
CorrectionMatrixCalculator::CalculationResult CorrectionMatrixCalculator::calculateMatrix(const std::vector<Types::ColorValue>& measuredColors,
...
CorrectionMatrixCalculator::CalculationResult CorrectionMatrixCalculator::refineMatrix(const Domain::CorrectionMatrix& initialMatrix,
...
void CorrectionMatrixCalculator::setSettings(const CalculationSettings& settings) {
    settings_ = settings;
}

CorrectionMatrixCalculator::CalculationSettings CorrectionMatrixCalculator::getSettings() const {
    return settings_;
}

Domain::CorrectionMatrix CorrectionMatrixCalculator::quickCalculate(const std::vector<Domain::ColorPatch>& patches) {
...
bool CorrectionMatrixCalculator::prepareDataMatrices(const std::vector<Domain::ColorPatch>& patches,
...
cv::Mat CorrectionMatrixCalculator::calculateLeastSquares(const cv::Mat& measured, const cv::Mat& reference) {
    // Solve: reference = measured * matrix^T
    // matrix^T = measured^+ * reference (where ^+ is pseudoinverse)
    
    if (measured.rows != reference.rows || measured.cols != 3 || reference.cols != 3) {
        LOG_ERROR("Matrix dimension mismatch in least squares: measured ", 
                 measured.rows, "x", measured.cols, ", reference ", 
    ...
    cv::Mat referenceT;
    cv::transpose(reference, referenceT);
    
    cv::Mat matrixT;
    cv::solve(measuredT, referenceT, matrixT, cv::DECOMP_NORMAL);
...
cv::Mat CorrectionMatrixCalculator::calculateSVDLeastSquares(const cv::Mat& measured, const cv::Mat& reference) {
    // Use SVD for more stable solution
    if (measured.rows != reference.rows || measured.cols != 3 || reference.cols != 3) {
        LOG_ERROR("Matrix dimension mismatch in SVD least squares: measured ", 
                 measured.rows, "x", measured.cols, ", reference ", 
                 reference.rows, "x", reference.cols);
        return cv::Mat();
    }
    ...
    // Reconstruct pseudoinverse: A+ = V * Σ+ * U^T
    // For m×n matrix, we need first min(m,n) columns of U
    int rank = std::min(measured.rows, measured.cols);
    
    // Create inverse diagonal matrix from singular values
    cv::Mat wInvMat = cv::Mat::zeros(rank, rank, CV_32F);
    for (int i = 0; i < w.rows; ++i) {
        wInvMat.at<float>(i, i) = 1.0f / w.at<float>(i, 0);
    }
    
    // Use only first 'rank' columns of U for pseudoinverse
    cv::Mat uReduced = u(cv::Rect(0, 0, rank, u.rows)); // First 'rank' columns
    
    if (vt.cols != wInvMat.rows || wInvMat.cols != uReduced.cols) {
        LOG_ERROR("SVD pseudoinverse dimension mismatch: vt ", vt.rows, "x", vt.cols,
                 ", wInvMat ", wInvMat.rows, "x", wInvMat.cols, 
                 ", uReduced ", uReduced.rows, "x", uReduced.cols);
        return cv::Mat();
    }
    
    cv::Mat pseudoInverse = vt.t() * wInvMat * uReduced.t();
    
    if (pseudoInverse.cols != reference.rows) {
        LOG_ERROR("Pseudoinverse dimension mismatch: pseudoInverse ", 
                 pseudoInverse.rows, "x", pseudoInverse.cols, 
                 ", reference ", reference.rows, "x", reference.cols);
        return cv::Mat();
    }
...
cv::Mat CorrectionMatrixCalculator::calculateWeightedLeastSquares(const cv::Mat& measured, const cv::Mat& reference, const cv::Mat& weights) {
...
cv::Mat CorrectionMatrixCalculator::calculateRidgeRegression(const cv::Mat& measured, const cv::Mat& reference) {
    // Ridge regression: (X^T X + α I)^-1 X^T y
    if (measured.rows != reference.rows || measured.cols != 3 || reference.cols != 3) {
        LOG_ERROR("Matrix dimension mismatch in ridge regression: measured ", 
                 measured.rows, "x", measured.cols, ", reference ", 
                 reference.rows, "x", reference.cols);
        return cv::Mat();
    }
    ...
    if (measuredT.cols != measured.rows) {
        LOG_ERROR("Transpose dimension error: measuredT ", measuredT.rows, "x", measuredT.cols,
                 ", expected ", measured.cols, "x", measured.rows);
        return cv::Mat();
    }
    ...
    if (XTX.rows != XTy.rows) {
        LOG_ERROR("Ridge regression solve dimension mismatch: XTX ", XTX.rows, "x", XTX.cols,
                 ", XTy ", XTy.rows, "x", XTy.cols);
        return cv::Mat();
    }
...
cv::Mat CorrectionMatrixCalculator::calculateRANSAC(const cv::Mat& measured, const cv::Mat& reference, CalculationResult& result) {
    ...
    for (int iter = 0; iter < settings_.ransacIterations; ++iter) {
        ...
        for (int i = 0; i < numSamples; ++i) {
            ...
            if (measuredRow.cols != candidateMatrix.rows) {
                LOG_DEBUG("RANSAC dimension mismatch: measuredRow ", measuredRow.cols, 
                         ", candidateMatrix ", candidateMatrix.rows, "x", candidateMatrix.cols);
                continue;
            }
...
void CorrectionMatrixCalculator::removeOutliers(cv::Mat& measured, cv::Mat& reference, cv::Mat& weights, CalculationResult& result) {
...
void CorrectionMatrixCalculator::calculateQualityMetrics(const cv::Mat& measured, const cv::Mat& reference, 
                           const cv::Mat& matrix, CalculationResult& result) {
    // Calculate residuals
    if (measured.cols != matrix.rows || measured.rows != reference.rows) {
        LOG_ERROR("Quality metrics dimension mismatch: measured ", measured.rows, "x", measured.cols,
                 ", matrix ", matrix.rows, "x", matrix.cols, ", reference ", reference.rows, "x", reference.cols);
        return;
    }
...
float CorrectionMatrixCalculator::performCrossValidation(const std::vector<Domain::ColorPatch>& patches) {
...
Types::ColorValue CorrectionMatrixCalculator::normalizeColor(const Types::ColorValue& color) {
