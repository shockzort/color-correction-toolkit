...
class QualityMetrics {
  public:
    ...
    struct ComparisonSettings {
        ...
        Types::ColorSpace workingColorSpace = Types::ColorSpace::LAB;
        
        // CIE94 parameters
        float cie94_kL = 1.0f;
        float cie94_kC = 1.0f;
        float cie94_kH = 1.0f;
        float cie94_K1 = 0.045f;
        float cie94_K2 = 0.015f;
        
        // CIEDE2000 parameters
        float ciede2000_kL = 1.0f;
        float ciede2000_kC = 1.0f;
        float ciede2000_kH = 1.0f;
        
        // CMC parameters
        float cmc_l = 2.0f;  // Lightness weight
        float cmc_c = 1.0f;  // Chroma weight
        
        // Quality thresholds
        float excellentThreshold = 1.0f;    // ΔE < 1.0 = excellent
        float goodThreshold = 2.0f;         // ΔE < 2.0 = good
        float fairThreshold = 4.0f;         // ΔE < 4.0 = fair
        
        // Statistical settings
        bool includeOutliers = true;
        float outlierThreshold = 3.0f;      // Standard deviations for outlier detection
        bool weightByConfidence = true;
    };

    QualityMetrics(const ComparisonSettings& settings = ComparisonSettings{})
