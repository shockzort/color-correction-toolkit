...
class LinearCorrector {
  public:
    struct CorrectionSettings {
        ...
        bool clampValues = true;            // Clamp output values to [0,1]
        bool preserveAlpha = true;          // Preserve alpha channel if present
        float qualityFactor = 1.0f;         // Quality vs speed trade-off (0.1-1.0)
        
        // Error handling
        bool enableErrorRecovery = true;    // Fallback to identity on errors
        bool validateInputs = true;         // Validate input images/matrices
        ...
        bool maintainInputType = true;      // Keep same bit depth as input
        bool enableDithering = false;       // Dithering for 8-bit output
        
        // Quality validation
        bool performQualityCheck = false;   // Check output quality
        float minQualityThreshold = 0.8f;   // Minimum acceptable quality
    };
