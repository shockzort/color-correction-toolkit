...
// BatchSettings constructor
BatchProcessor::BatchSettings::BatchSettings()
    : enableParallelProcessing(true),
      ...
      supportedExtensions({".jpg", ".jpeg", ".png", ".bmp", ".tiff", ".tif"}),
      recursiveDirectorySearch(false),
      preloadCalibration(true),
      enableIOOptimization(true) {}

// BatchResult::getSummary
std::string BatchProcessor::BatchResult::getSummary() const {
...
BatchProcessor::BatchProcessor(const BatchSettings& settings)
...
// Process multiple image files (CalibrationData overload)
BatchProcessor::BatchResult BatchProcessor::processFiles(
...
// Process multiple image files (CorrectionMatrix overload)
BatchProcessor::BatchResult BatchProcessor::processFiles(
    const std::vector<std::string>& inputFiles,
    const std::string& outputDirectory,
...
BatchProcessor::BatchResult BatchProcessor::processDirectory(
...
std::vector<Types::Image> BatchProcessor::processImageBatch(
...
void BatchProcessor::setProgressCallback(ProgressCallback callback) {
    progressCallback_ = callback;
}

void BatchProcessor::setErrorCallback(ErrorCallback callback) {
    errorCallback_ = callback;
}

// Settings management
void BatchProcessor::setSettings(const BatchSettings& settings) {
    settings_ = settings;
    corrector_.setSettings(createCorrectorSettings(settings));
}

BatchProcessor::BatchSettings BatchProcessor::getSettings() const {
    return settings_;
}

// Utility methods
std::vector<std::string> BatchProcessor::findImageFiles(const std::string& directory, 
...
float BatchProcessor::estimateBatchProcessingTime(const std::vector<std::string>& inputFiles) const {
...
// Private methods
LinearCorrector::CorrectionSettings BatchProcessor::createCorrectorSettings(const BatchSettings& batchSettings) {
...
void BatchProcessor::createOutputDirectory(const std::string& outputDirectory) {
    try {
...
std::vector<std::string> BatchProcessor::filterValidFiles(const std::vector<std::string>& inputFiles, 
                                        BatchResult& result) {
...
BatchProcessor::BatchResult BatchProcessor::processFilesSequential(const std::vector<std::string>& files,
                                 const std::string& outputDirectory,
                                 const Domain::CorrectionMatrix& correctionMatrix) {
    BatchResult result;
    result.outputDirectory = outputDirectory;
...
BatchProcessor::BatchResult BatchProcessor::processFilesParallel(const std::vector<std::string>& files,
                               const std::string& outputDirectory,
                               const Domain::CorrectionMatrix& correctionMatrix) {
    BatchResult result;
    result.outputDirectory = outputDirectory;
...
bool BatchProcessor::processSingleFile(const std::string& inputFile,
                      const std::string& outputDirectory,
                      const Domain::CorrectionMatrix& correctionMatrix,
                      BatchResult& result) {
    try {
...
bool BatchProcessor::saveImage(const Types::Image& image, const std::string& outputPath) {
    try {
...
void BatchProcessor::updateProgress(int current, int total, const std::string& currentFile, 
...
void BatchProcessor::reportError(const std::string& filename, const std::string& error) {
    if (errorCallback_) {
